...
public class DepthMapConfirmDialog extends DialogFragment {
	 ...
	 * callback interface try { // Instantiate the NoticeDialogListener so we
	 * can send events to the host mListener = (NoticeDialogListener) activity;
	...
	public Dialog onCreateDialog(Bundle savedInstanceState) {
		...
		ViewGroup vg = (ViewGroup) inflater.inflate(R.layout.depthmap_confirm,
				null);
		...
		builder.setPositiveButton(R.string.Accept,
				new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						Runnable serverAction = new Runnable() {
							public void run() {
								...
								Server.sendFrame(colorMat, 1, (double) -1
										* colorMat.cols() / 2, (double) -1
										* colorMat.rows() / 2, 112,
										-.06666666666, 4);
								...
								MichelangeloCamera.saveThumbnail(
										depthpair.thumbnail, fileName + ".jpg");
								Server.close();
							}
						};
						Thread myThread = new Thread(serverAction);
						myThread.start();
					}
				});
